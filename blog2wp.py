...
def prevoddata(fujdatum, debug):
    ...
    if '.' in fujdatum or ':' in fujdatum:
        ...
        cas = str(pred.ctime)
        wpdatum = str(rok) + '-' + str(mesic) + '-' + str(den) + \
...
def stahni_obrazek(url_ke_stazeni, export_mode, debug, prepis_obrazku):
    ...
    if 'bcache' in vstupni_url or 'imageproxy' in vstupni_url:
        ...
        if debug:
            sys.stdout.write(
                'spravna url po zruseni ochrany: ' +
                spravna_url +
                '\n')
        ...
        # je zjistena url rovnou povazovana za pouzitelnou
        spravna_url = vstupni_url

    # samotne stahovani neprovadime u varianty exportu, kdy ponechavame
    # puvodni url obrazku
    if export_mode != 3:
...
def exportovat_rubriky(url_blog, vystupni_soubor, xml_filename_count, debug):
    ...
    export_pubdate = datetime.datetime.strftime(
        aktualni_timestamp, '%a, %d %b %Y %H:%M:%S')
    export_pubdate = export_pubdate + ' +0000'
    export_created = datetime.datetime.strftime(
        aktualni_timestamp, '%Y-%m-%d %H:%M:%S')

    if xml_filename_count == 1:
        if os.path.isfile('temp/rubriky'):
            sys.stdout.write(
                status_warn +
                'soubor temp/rubriky již existuje, pokračuji s daty z něho\n')
        else:
            stahni_html(url_rubriky, False)
            sys.stdout.write(
                status_ok +
                'soubor s definicemi rubrik úspešně stažen\n')
    ...
    with open('temp/rubriky', 'r') as k:
        for line in k:
            ...
            if "<title>" in line:
                ...
                wpxml.write(
                    '<!-- generator="Blogcz2WP ver.' +
                    verze +
                    '" created="' +
                    export_created +
                    '" -->\n')
                wpxml.write(
                    '<!-- Blogcz2WP: blog.cz to Wordpress export tool, made by Martin Rybensky 2017-2020  -->\n')
                wpxml.write(
                    '<!-- Project homepage: https://github.com/MartinRybensky/blogcz2wp  -->\n')
                ...
                wpxml.write(
                    '     <wp:author_login><![CDATA[' + autor_blogu + ']]></wp:author_login>\n')
                wpxml.write(
                    '     <wp:author_email><![CDATA[]]></wp:author_email>\n')
                wpxml.write(
                    '     <wp:author_display_name><![CDATA[' +
                    autor_blogu +
            ...
            if cislo_radku == radek_rubrika:
                rubrika_url[cislo_rubriky] = line.split(
                    'href="/rubrika/', 1)[1]
                rubrika_url[cislo_rubriky] = rubrika_url[cislo_rubriky].split('"', 1)[
                    0]

                rubrika_nazev[cislo_rubriky] = line.split('title="', 1)[1]
                rubrika_nazev[cislo_rubriky] = rubrika_nazev[cislo_rubriky].split('">', 1)[
                    0]
                ...
                if debug:
                    sys.stdout.write(
                        ' ' + str(cislo_rubriky) + ': ' + rubrika_nazev[cislo_rubriky] + '\n')
...
def exportovat_archiv(url_blog, debug):
    ...
    if os.path.isfile('temp/archiv'):
        sys.stdout.write(
            status_warn +
            'soubor temp/archiv již existuje, pokračuji s daty z něho\n')
    else:
    ...
    with open('temp/archiv', 'r') as k:
        for line in k:
            ...
            if cislo_radku == radek_odkaz:
                if debug:
                    sys.stdout.write(
                        'exportovat_archiv, zpracovavany radek: ' +
                        str(cislo_radku) +
                        '\n')
            ...
            if cislo_radku == radek_pocet:
                ...
                if debug:
                    sys.stdout.write(
                        archiv_nazev[zaznam] + ', záznamů: ' + str(archiv_pocet[zaznam]) + '\n')
    ...
    sys.stdout.write(
        ' [info] celkový počet článků v archivu (dle deklarovaných počtů v závorkách): ' +
        str(pocet_celkem) +
        '\n')
...
def soupis_clanku(url_blog, debug):
    ...
    if os.path.isfile('temp/soupis_clanku.txt'):
        sys.stdout.write(
            status_warn +
            'soubor temp/soupis_clanku.txt již existuje, pokračuji s daty z něho\n')
        with open('temp/soupis_clanku.txt', 'r') as s:
            for radek in s:
                if url_blog not in radek:
                    sys.stdout.write(
                        status_fail +
                        'neodpovídající lokální data, soupis_clanku.txt obsahuje odkazy na jiný blog!\n        Program zavolán ve špatném adresáři nebo na špatnou URL?\n')
                    sys.exit(1)
    else:
            ...
            for radek in f:
                ...
                with open('temp/' + soubor_rubrika, 'r') as p:
                    for line in p:
                        ...
                        # sebere odkazy na clanky na prvni/jedine strance
                        # vypisu rubriky
                        ...
                        # pokud je v rubrice hodne clanku a strankuje se
                            ...
                            strankovani = True
                            posledni_stranka = line.rsplit(
                                '" title="Poslední stránka"', 1)[0]
                            posledni_stranka = posledni_stranka.rsplit('"', 1)[
                                1]
                            posledni_stranka = posledni_stranka.rsplit(
                                '/', 1)[1]
                            if debug:
                                sys.stdout.write(
                                    'posledni stránka katogorie: ' + posledni_stranka + '\n')

                            for_posledni = int(posledni_stranka)
                            for_posledni = for_posledni + 1

                            for y in range(2, for_posledni):
                                strankovana_rubrika = url_blog + \
                                    '/' + soubor_rubrika + '/' + str(y)
                                ...
                                os.rename(
                                    ...
                                    str(y) +
                                    '.txt')
                                nove_jmeno = 'temp/' + soubor_rubrika + \
                                    '-' + str(y) + '.txt'
                                ...
                                with open(nove_jmeno, 'r') as r:
                                    for line in r:
                                        ...
                                        # sebere odkazy na clanky na
                                        # prvni/jedine strance vypisu rubriky
...
def zpracovat_komentare(vstupni_soubor, vystupni_soubor, debug):
    ...
    with open(vstupni_soubor, 'rU') as c:
        for lajnc in c:
            if 'discussPrevious' in lajnc:
                ...
                if debug:
                    sys.stdout.write(
                        'discussPrevious nalezen (strankovany vypis komentaru detekovan)\n')
    ...
    with open(vstupni_soubor, 'rU') as d:
        for lajnd in d:
            if 'commentNr" id="ref1"' in lajnd:
                ...
                if debug:
                    sys.stdout.write(
                        'commentNr" id="ref1" nalezen (vypis komentaru od 1. detekovan)\n')
    ...
    with open(vstupni_soubor, 'rU') as e:
        for lajn in e:
            ...
            if strankovane_komentare and not od_jednicky and 'discussPrevious' in lajn:
                ...
                if debug:
                    sys.stdout.write(
                        'zpracovavam zbytek z clanku, pocatecni index: ' +
                        str(index_pole) +
                        '\n')
                zapis_komentare(
                    vstupni_soubor,
    ...
    if not zpracovano:
        index_pole = int(0)
        # nestrankovane komentare / zbytek komentaru prebyvajicich ze
        # strankovanych
        komentarovy_soubor = vstupni_soubor
        if debug:
            sys.stdout.write(
                'komentare zpracovavany ze souboru: ' +
                komentarovy_soubor +
                '\n')
    ...
    with open(komentarovy_soubor, 'rU') as b:
        for line in b:
            ...
            if '<a class="commentNr" id="ref' in line:
                ...
                pocet_komentaru = line.split(
                    '<a class="commentNr" id="ref', 1)[1]
                pocet_komentaru = pocet_komentaru.split('" name=', 1)[0]
                pocet_komentaru = int(pocet_komentaru)
                if debug:
                    sys.stdout.write(
                        'pocet komentaru: ' + str(pocet_komentaru) + '\n')
                kom_jmeno[pocet_komentaru] = line.split('<strong>', 1)[1]
                kom_jmeno[pocet_komentaru] = kom_jmeno[pocet_komentaru].split(
                    '</strong>', 1)[0]

                # mail + web
                if 'mailto:' in line and 'title="Web"' in line:
                    kom_email[pocet_komentaru] = line.split('mailto:', 1)[1]
                    kom_email[pocet_komentaru] = kom_email[pocet_komentaru].split(
                        '">E-mail', 1)[0]
                    kom_email[pocet_komentaru] = kom_email[pocet_komentaru].replace(
                        ' (V) ', '@')

                    kom_web[pocet_komentaru] = line.split(
                        'E-mail</a> | <a href="', 1)[1]
                    kom_web[pocet_komentaru] = kom_web[pocet_komentaru].split('" title="Web">', 1)[
                        0]
                    kom_web[pocet_komentaru] = ocistit_url(
                        kom_web[pocet_komentaru])

                    kom_datum[pocet_komentaru] = line.split('Web</a> | ', 1)[1]
                    kom_datum[pocet_komentaru] = kom_datum[pocet_komentaru].split(' | <a', 1)[
                        0]
                # jen web
                ...
                # jen mail
                    ...
                    kom_datum[pocet_komentaru] = line.split(
                        'E-mail</a> | ', 1)[1]
                    kom_datum[pocet_komentaru] = kom_datum[pocet_komentaru].split(' | <a', 1)[
                        0]
                # ani jedno
            ...
            # text komentare
                ...
                if debug:
                    sys.stdout.write(
                        'cislo_radku_komentare: ' +
                        str(cislo_radku_komentare) +
                        ' ; radek_s_komentarem: ' +
                        str(radek_s_komentarem) +
                        '\n')
                kom_text[pocet_komentaru] = line.split('<p>', 1)[1]
                kom_text[pocet_komentaru] = kom_text[pocet_komentaru].split(
                    '</p></div></div>', 1)[0]
                ...
                if debug:
                    sys.stdout.write(
                        'Detekován víceřádkový komentář. nasledujici_radek_komentare: ' +
                        str(nasledujici_radek_komentare) +
                        '\n')
...
def exportovat_clanek(
    ...
    with open(vstupni_soubor, 'rU') as k:
        for line in k:
            ...
            if '<meta property="og:url"' in line:
                if debug:
                    sys.stdout.write(
                        'meta property detekovano na radku: ' +
                        str(cislo_radku) +
                        '\n')
            ...
            # cislo radku, kde zacina clanek
                ...
                if debug:
                    sys.stdout.write(
                        'zacatek sekce <div class="article"> detekovan na radku: ' +
                        str(cislo_radku) +
                        '\n')
            # titulek clanku
            elif "<title>" in line:
            ...
            # slovni nazev rubriky
                ...
                if debug:
                    sys.stdout.write(
                        'nazev rubriky, ve ktere byl clanek publikovan: ' + rubrika + '\n')
            # autor clanku
                ...
                # ulozime si cislo radku, za kterym zacina samotny clanek + 1,
                # kde zacina doopravdy
            elif '<div class="articleText">' in line:
                radek_clanek = cislo_radku + 1
                if debug:
                    sys.stdout.write(
                        'cislo radku, kde je text clanku: ' +
                        str(radek_clanek) +
                        '\n')
            # tady uz samotny text..
                ...
                if debug:
                    sys.stdout.write(
                        'prave je zpracovavan radek s textem clanku\n')
               ...
               # vykosteni a stazeni obrazku
                        ...
                        for xx in line:
                            ...
                            try:
                                ...
                                if obrazek == obrazek:
                                    ...
                                    if export_mode == 4:
                                        stahni_obrazek(
                                            obrazek, export_mode, debug, prepis_obrazku)
                                        novy_blog_img = novy_blog.split('.', 1)[
                                            0]
                                        novy_blog_img = novy_blog_img + '.files.wordpress.com'
                                        nova_url = novy_blog_img + '/' + tento_rok + '/' + \
                                            tento_mesic.zfill(2) + '/' + jmeno_souboru
                                    if export_mode == 3:
                                        prepis_obrazku = 1
                                        nova_url = stahni_obrazek(
                                            obrazek, export_mode, debug, prepis_obrazku)
                                    if export_mode == 2:
                                        stahni_obrazek(
                                            obrazek, export_mode, debug, prepis_obrazku)
                                    ...
                                    # rewrite pouze adres obrázků hostovaných
                                    # na blog.cz
                                    if 'jxs.cz' in obrazek and prepis_obrazku == 2:
                                        text_clanku = text_clanku.replace(
                                            obrazek, nova_url)
                                    ...
                                    if debug:
                                        sys.stdout.write(
                                            'obrázek již stažen a zpracován\n')
                            except BaseException:
                                break

                # ------- rewrite odkazů na interní články (na stejném blogu, j
                odkaz = ''
                odkaz_novy = ''
                ...
                if url_blog in text_clanku:
                    if debug:
                        sys.stdout.write(
                            'v článku byl nalezen odkaz na interní článek\n')
                    tt = '<a href="'
                    pp = 0
                    konec_odkazu = False
                    ...
                    if not konec_odkazu:
                        ...
                        for tt in ke_zpracovani:
                            ...
                            try:
                                ...
                                if posledni_odkaz == odkaz:
                                    if debug:
                                        sys.stdout.write(
                                            'tento odkaz byl poslední, konec cyklu \n')
                                    break
                                if debug:
                                    sys.stdout.write(
                                        str(pp) + ': ' + odkaz + '\n')
                                ...
                                # a zároveň pouze pokud je aktivní režim
                                # přepisu odkazů
                                if url_blog in odkaz and prepis_odkazu == 2:
                                    ...
                                    if debug:
                                        sys.stdout.write(
                                            'odkaz novy: ' + odkaz_novy + '\n')

                                posledni_odkaz = odkaz

                                # odzriznout jiz zpracovanou cast
                                ke_zpracovani = ke_zpracovani.split(odkaz, 1)[
                                    1]
                            except BaseException:
                                break
                    else:
                        if debug:
                            sys.stdout.write(
                                'nebyl nalezen žádný interní odkaz k přepsání\n')
    ...
    if autor == "":
        if debug:
            sys.stdout.write(
                ...
                'U článku "' +
                titulek_clanku +
                '" se nepodařilo extrahovat jméno autora\n')
        autor = "neznámý autor"
        ...
        sys.stdout.write(
            ...
            'U článku "' +
            titulek_clanku +
            '" se nepodařilo extrahovat datum publikování\n')

    elif text_clanku == "":
        sys.stdout.write(
            '\n' +
            status_warn +
            'Nepodařilo se extrahovat text článku "' +
            titulek_clanku +
            '"\n')
    ...
    wpxml.write('       <category domain="category" nicename="' +
                rubrika_url + '"><![CDATA[' + rubrika + ']]></category>\n')
...
sys.stdout.write('\n')
sys.stdout.write(
    '1 - vlastní wordpress nebo hosting obrázků: stáhnout obrázky, cesty změnit na absolutní\n')
sys.stdout.write(
    '2 - vlastní wordpress: stáhnout obrázky, cesty změnit na relativní do wp-content/uploads\n')
sys.stdout.write(
    '3 - blogspot.com: obrázky nestahovat, ponechat absolutní cesty na jejich puvodní umístění\n')
sys.stdout.write(
    '4 - wordpress.com: stáhnout obrázky, cesty změnit na nové umistaní na wp.com\n')

while True:
    export_mode_vstup = input("Zvolte režim exportu (1-4): ")
...
if export_mode == 1:
    sys.stdout.write(
        'Zadejte doménu, na které bude běžet nový blog.\nZadávejte ve tvaru s http/https, např. http://mojedomena.cz nebo https://mujblog.cz\n')
    while True:
        novy_blog = input("Adresa: ")
        if novy_blog != '':
            break
    sys.stdout.write(
        'Zadej URL adresáře s obrázky, cesta může být absolutní i relativní - např. http://mojedomena.cz/obrazky/fotky, img/obrazky nebo wp-content/uploads/stare\n')

    while True:
        img_dir = input("Adresa: ")
        if img_dir != '':
            break
...
if export_mode != 3:
    ...
    sys.stdout.write(
        'Režim nakládání s interními odkazy směrujícími na články na exportovaném blogu:\n')
    sys.stdout.write(
        '1 - ponechat v původní podobě, např. <a href="' +
        url_blog +
        '/1902/nejaky-pekny-clanek">\n')
    sys.stdout.write(
        '2 - přepsat na novou doménu s konvencí Permalink: post name, např. <a href=' +
        novy_blog +
        '/nejaky-pekny-clanek">\n')
    while True:
        prepis_odkazu_input = input("Zvolte režim (1-2):\n")
...
if export_mode != 3:
    ...
    sys.stdout.write(
        'Režim nakládání s adresami obrázků linkovaných v článcích:\n')
    sys.stdout.write(
        '1 - přepsat všechny cesty na nové umístění, stáhnout všechny obrázky\n')
    sys.stdout.write(
        '2 - přepsat pouze cesty obrázků hostovaných na blog.cz a stáhnout pouze ty\n')
    while True:
        prepis_obrazku_input = input("Zvolte režim (1-2):\n")
...
# porovnavame pocet radku s odkazy na clanky s cislem spocitanym z poctu
# clanku v archivu
pocet_dle_souboru = num_lines = sum(
    1 for line in open('temp/soupis_clanku.txt'))

# pokud program nebezi v debug modu, pri nesouhlasu poctu nalezenych
# clanku oproti archivu se ukonci
if pocet_dle_souboru != pocet_dle_blogu:
    sys.stdout.write(
        status_fail +
        'Počet článků nesouhlasí! V archivu deklarováno / nalezeno: ' +
        str(pocet_dle_blogu) +
        ' / ' +
        str(pocet_dle_souboru) +
        '\n')
    if not debug:
        sys.exit(1)
else:
    sys.stdout.write(
        '\n' +
        status_ok +
        'Počet nalezených článků se shoduje s deklarovanými počty v archivu\n')
    sys.stdout.write(
        status_ok +
        'Celkový počet článků ke stažení: ' +
        str(pocet_dle_souboru) +
        '\n')
    sys.stdout.write(
        ' [info] stahuji články, zpracovávám jejich obsah a zapisuji jej do XML:\n')
    ...
    for zaznam in m:
        ...
        if xml_filesize.st_size <= xml_max_filesize:
            ...
            if os.path.isfile(clanek_soubor_test):
                if debug:
                    sys.stdout.write(
                        ...
                        '/' +
                        str(pocet_dle_souboru) +
                        '] soubor již existuje, zapisuji do xml: ' +
                        clanek_soubor +
                        '\n')
                else:
            ...
            sys.stdout.write(
                ...
                'Výstupní soubor ' +
                vystupni_soubor +
                ' překročil velikost 5 MB, pokračuji do nového\n')

            xml_filename_count += 1

            vystupni_soubor = xml_filename_noext + '_' + \
                str(xml_filename_count).zfill(2) + '.xml'

            # do nově definovaného následující výstupního souboru zapíšeme
            # hlavičku
            exportovat_rubriky(
                url_blog,
                vystupni_soubor,
...
if xml_filename_count == 1:
    ...
    sys.stdout.write(
        '\n' +
        status_ok +
        'Hotovo! Výsledek uložen do celkem ' +
        str(xml_filename_count) +
        ' XML souborů\n')
if export_mode == 3:
    sys.stdout.write(
        ' XML je třeba před importem do Blogspotu zkonvertovat pomocí této webové aplikace:\n')
    sys.stdout.write(
